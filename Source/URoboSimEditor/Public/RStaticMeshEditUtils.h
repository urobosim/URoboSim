...
{
public:
  static UStaticMesh *CreateStaticMesh(UPackage *InPackage, const FString &InPackageName, const USDFGeometry *InGeometry);
  // template <typename T>
  ...
  static void GenerateKDop(UStaticMesh *&OutMesh, const ECollisionType &InCollisionType);
  static void GenerateKDop(UStaticMesh *&OutMesh, const FVector *InDirections, const uint32 &InNumInDirections);
  //   static TArray<float> GetGeometryParameter(USDFGeometry *InGeometry);

protected:
  static UProceduralMeshComponent *CreateCubePM(UObject *InOwner, const TArray<float> &InParameter);
  static UProceduralMeshComponent *CreateCylinderPM(UObject *InOwner, const TArray<float> &InParameter, const float &Section = 8);
