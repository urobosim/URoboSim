...
DEFINE_LOG_CATEGORY_STATIC(LogRStaticMeshUtils, Log, All);

UStaticMesh *RStaticMeshUtils::CreateStaticMesh(UPackage *InPackage, const FString &InPackageName, const USDFGeometry *InGeometry)
{
  ...
  case ESDFGeometryType::None:
    UE_LOG(LogRStaticMeshUtils, Error, TEXT("GeometryType not implemented"));
    break;
  case ESDFGeometryType::Mesh:
    UE_LOG(LogRStaticMeshUtils, Error, TEXT("Mesh not implemented"));
    break;
  case ESDFGeometryType::Box:
    Parameters.Add(Cast<USDFGeometryBox>(InGeometry)->Size.X);
    Parameters.Add(Cast<USDFGeometryBox>(InGeometry)->Size.Y);
    Parameters.Add(Cast<USDFGeometryBox>(InGeometry)->Size.Z);
    ProcMeshComp = CreateCubePM(InPackage, Parameters);
    break;
  case ESDFGeometryType::Cylinder:
    Parameters.Add(Cast<USDFGeometryCylinder>(InGeometry)->Radius);
    Parameters.Add(Cast<USDFGeometryCylinder>(InGeometry)->Length);
    ProcMeshComp = CreateCylinderPM(InPackage, Parameters);
    break;
  case ESDFGeometryType::Sphere:
    Parameters.Add(Cast<USDFGeometrySphere>(InGeometry)->Radius);
    Parameters.Add(Cast<USDFGeometrySphere>(InGeometry)->Radius);
    Parameters.Add(Cast<USDFGeometrySphere>(InGeometry)->Radius);
    ProcMeshComp = CreateCubePM(InPackage, Parameters);
    UE_LOG(LogRStaticMeshUtils, Error, TEXT("GeometryType Sphere for %s not implemented, use Cube instead."), *InPackage->GetName());
    break;
  }
...
void RStaticMeshUtils::GenerateKDop(UStaticMesh *&OutMesh, const ECollisionType &InCollisionType)
{
...
void RStaticMeshUtils::GenerateKDop(UStaticMesh *&OutMesh, const FVector *InDirections, const uint32 &InNumInDirections)
{
...
UProceduralMeshComponent *RStaticMeshUtils::CreateCubePM(UObject *InOwner, const TArray<float> &InParameter)
{
...
UProceduralMeshComponent *RStaticMeshUtils::CreateCylinderPM(UObject *InOwner, const TArray<float> &InParameter, const float &Section)
{
  UProceduralMeshComponent *ProcMeshComp = NewObject<UProceduralMeshComponent>(InOwner);
  TArray<FVector> Vertices;
  TArray<int32> Triangles;
