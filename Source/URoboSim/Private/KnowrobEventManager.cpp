...
void UCollisionEventManager::Callback(AActor* SelfActor, AActor* OtherActor, FVector NormalImpulse, const FHitResult& Hit)
{
  if(KnowrobInterface)
    {
      FString Query = TEXT("get_time(Now),tell([is_event(E),has_type(E,failure:'PlacementFailure'),is_diagnosis(D), has_type(D, failure:'CollisionWithMovableObject'),has_participant(E, D),has_participant(O1, O2),is_time_interval(Interval),holds(Interval, soma:hasIntervalBegin, Begin),holds(Interval, soma:hasIntervalEnd, Now)]).");
