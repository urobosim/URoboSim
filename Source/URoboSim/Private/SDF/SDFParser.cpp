...
void FSDFParser::ParseVisual(const FXmlNode* InNode, USDFLink*& OutLink)
{
	...
	for (const auto& ChildNode : InNode->GetChildrenNodes())
	{
		...
		else if (ChildNode->GetTag().Equals(TEXT("geometry")))
		{
			...
			// {
			// 	// RStaticMeshUtils::CreateStaticMeshThroughBrush(OutLink,NewVisual);
			// }
		}
		else
		{
...
UStaticMesh* FSDFParser::ImportMesh(const FString& Uri, ESDFType Type)
{
	...
	// Get mesh name
	FName MeshName;
	if (Type == ESDFType::Collision)
	{
		MeshName = FName(*(TEXT("SM_") + FPaths::GetBaseFilename(MeshAbsolutePath) + TEXT("_C")));
	}
	else if (Type == ESDFType::Visual)
	{
		MeshName = FName(*(TEXT("SM_") + FPaths::GetBaseFilename(MeshAbsolutePath) + TEXT("_V")));
	}

	// Import mesh
	bool bOperationCancelled = false;

	// UE_LOG(LogTemp, Error, TEXT("OuterName %s"), *DataAsset->GetOuter()->GetName());
	// UE_LOG(LogTemp, Error, TEXT("OuterClass %s"), *DataAsset->GetOuter()->GetClass()->GetName());
...
FTransform FSDFParser::PoseContentToFTransform(const FString& InPoseData)
{
	...
	// TODO right to left handed coordinate system
	if (ArrSize == 6)
	{
		// const FVector Loc = FConversions::MToCm(
		// 		FVector(FCString::Atof(*PoseDataArray[0]),
		...
		// const FRotator Rot(FMath::RadiansToDegrees(FCString::Atof(*PoseDataArray[3]) < 0 ? FCString::Atof(*PoseDataArray[3]) + 2*PI : FCString::Atof(*PoseDataArray[3])),	// pitch
		// 				FMath::RadiansToDegrees(FCString::Atof(*PoseDataArray[4])< 0 ? FCString::Atof(*PoseDataArray[4]) + 2*PI : FCString::Atof(*PoseDataArray[4])),		// yaw
		// 				FMath::RadiansToDegrees(FCString::Atof(*PoseDataArray[5])< 0 ? FCString::Atof(*PoseDataArray[5]) + 2*PI : FCString::Atof(*PoseDataArray[5])));		// roll
